...
- Support **auto-detection** of sensible defaults (e.g. host name, process/service identity on servers; device/app identity on-device) where cheaply available on each platform, and allow **explicit overrides** that take precedence over detection. **RESOLVED (confirmed against .NET `ResourceExtensions.CreateAzureMonitorResource`):** `ai.cloud.role` = `"[{service.namespace}]/{service.name}"` when a namespace is present (bracketed namespace, `/` separator — the exact .NET form), else the bare `service.name`, else omitted. `service.version` → `ai.application.ver`. Two deliberate divergences from the server-only .NET exporter, both spec-driven: `ai.cloud.roleInstance` falls back to the `host.name` **resource attribute** (not a live `gethostname()` syscall — live identity arrives via the OTel `Resource` detectors or an explicit override), and `ai.device.*` are mapped from the OTel `device.*`/`os.*` resource attributes because Stout runs on-device (.NET, server-side, does not).
...
- ~~Exact role-name composition rule for `service.namespace` + `service.name` → `ai.cloud.role`.~~ **RESOLVED (confirmed against .NET `ResourceExtensions`):** `"[{namespace}]/{name}"` when a namespace is present, else bare `{name}`, else omitted. See the Resource detection FR above.
