...
- [X] T024 [P] [US4] `IngestionResponseTests` in `Tests/StoutCoreTests/IngestionResponseTests.swift`: parse `itemsReceived`/`itemsAccepted`/per-item `errors`; empty/non-JSON/malformed body → non-fatal, no crash (Acc #8; FR-024).
- [X] T025 [P] [US4] `RetryClassificationTests` in `Tests/StoutCoreTests/RetryClassificationTests.swift`: whole-response retriable `{408,429,439,401,403,500,502,503,504}`; 206 per-item retriable `{408,429,439,500,503}` (others dropped); `400/402/404` never retried; `401/403` retried then exhausted; `Retry-After` (delta-seconds + HTTP-date) honored; else full-jitter backoff `random(0, min(60s, 1s × 2^attempt))` bounded to 3 attempts / 60s (Acc #8/#9; FR-025/026/027).
- [X] T041 [P] [US4] `HostIsolationTests` in `Tests/StoutCoreTests/HostIsolationTests.swift`: a mock `Transport` that throws, plus malformed/garbage response bodies → `submit` never throws or blocks, the pipeline never crashes, failures surface ONLY via `Diagnostics`, and retriable errors exhaust the bounded budget then drop (spec Edge Case; FR-031; addresses analysis C1 — the consolidated do-no-harm assertion for Constitution Principle II).

### Implementation for User Story 4

- [X] T026 [P] [US4] Implement `IngestionResponse` parsing (`itemsReceived`/`itemsAccepted`/`errors[index,statusCode,message]`, malformed-body tolerant) in `Sources/StoutCore/Ingestion/IngestionResponse.swift` (FR-024; make T024 pass).
- [X] T027 [US4] Implement `RetryPolicy` — status classification (mirror .NET sets), `Retry-After` parsing (delta-seconds + HTTP-date), and **full-jitter exponential backoff `delay = random(0, min(maxRetryDelay, 1s × 2^attempt))`** bounded by `maxRetryAttempts` (3) / `maxRetryDelay` (60s) — in `Sources/StoutCore/Pipeline/RetryPolicy.swift` (FR-025/026/027; addresses analysis A1; depends on T026; make T025 pass).
- [X] T028 [US4] Wire `RetryPolicy` into the `ExportPipeline` export loop: retry retriable items (in-memory, bounded), re-queue 206 partial-success survivors, drop permanents with secret-free diagnostics in `Sources/StoutCore/Pipeline/ExportPipeline.swift` (FR-025; extends T021; depends on T027).
...
- [x] T036 Run a Swift 6 strict-concurrency audit: build `StoutCore` on an Apple platform AND Linux with zero data-race warnings, no `@unchecked Sendable` without justification (Acc #12; FR-030; SC-007) — pairs with a `swift6-concurrency-auditor` review. **Both legs verified 2026-07-11**: macOS (host Swift 6.3) and Linux (`swift:6.2` Docker) build clean under strict-concurrency-complete with zero data-race warnings; sole `@unchecked Sendable` is the test-only `RecordingDiagnostics`, justified inline. (One unrelated `ResourceAttributes` deprecation warning from the floated OpenTelemetryApi — not a data race; tracked as a follow-up.)
- [x] T037 [P] Run `swift format lint --strict --recursive Sources Tests` and fix violations (2-space indent).
- [x] T038 Execute the `quickstart.md` validation scenarios (1–13) on BOTH an Apple platform (iOS Simulator / macOS) and Linux; record results. **Verified 2026-07-11**: scenarios 1–13 are the XCTest suite — 92 tests pass on macOS (host Swift 6.3) AND on Linux (`swift:6.2` Docker with `zlib1g-dev`); iOS-Simulator build covered by the green CI `Build (iOS Simulator)` leg. `swift format lint --strict` clean on both.
